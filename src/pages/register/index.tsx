import {
  Box,
  Container,
  Paper,
  Typography,
  Button,
  Grid,
  Link,
  Avatar,
  FormControl,
  FormHelperText,
} from "@mui/material";
import { Link as RouterLink } from "react-router-dom";
import InputAdornment from "@mui/material/InputAdornment";
import IconButton from "@mui/material/IconButton";
import Visibility from "@mui/icons-material/Visibility";
import VisibilityOff from "@mui/icons-material/VisibilityOff";
import InputLabel from "@mui/material/InputLabel";
import OutlinedInput from "@mui/material/OutlinedInput";
import React, { useState } from "react";
import { keyframes } from "@mui/system";
import PersonAddOutlinedIcon from "@mui/icons-material/PersonAddOutlined";
import * as yup from "yup";

import { useNavigate } from "react-router-dom";
import { useAuth } from "../../context/AuthContext";
import { RegisterValidate, type RegisterFormData } from "../../utils/ValidateForm";

const fadeInUp = keyframes`
  from { 
    opacity: 0; 
    transform: translateY(20px); 
  }
  to { 
    opacity: 1; 
    transform: translateY(0); 
  }
`;

const RegisterPage = () => {
  const { register } = useAuth();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<RegisterFormData>({ // Using imported type
    username: "",
    email: "",
    password: "",
  });
  const [errors, setErrors] = useState<Partial<Record<keyof RegisterFormData, string>>>({});

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setFormData({
      ...formData,
      [name]: value,
    });
    setErrors((current) => ({
      ...current,
      [name]: undefined,
    }));
  };

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);

    try {
      await RegisterValidate.validate(formData, { abortEarly: false });
      setErrors({});
      console.log("Datos del formulario:", formData);
      await register(formData);
      navigate("/login"); // Redirige al login después del registro exitoso
    } catch (error: unknown) { // Removed direct yup import as it's not used
      if (error instanceof yup.ValidationError) {
        const validationErrors: Partial<Record<keyof RegisterFormData, string>> = {};
        error.inner.forEach((err) => {
          if (err.path) validationErrors[err.path as keyof RegisterFormData] = err.message;
        });
        setErrors(validationErrors);
      } else {
        console.error("Error en registro:", error);
      }
    } finally {
      setLoading(false);
    }
  };

  const [showPassword, setShowPassword] = React.useState(false);
  const handleClickShowPassword = () => setShowPassword((show) => !show);

  return (
    <Container maxWidth="xs" sx={{ 
      minHeight: '100vh', 
      display: 'flex', 
      alignItems: 'center', 
      justifyContent: 'center',
      animation: `${fadeInUp} 0.6s ease-out forwards`
    }}>
      <Paper elevation={0} sx={{ 
        padding: 4, 
        borderRadius: 4, 
        border: '1px solid', 
        borderColor: 'divider',
        width: '100%',
        boxShadow: '0 4px 20px rgba(0,0,0,0.05)'
      }}>
        <Avatar sx={{ mx: "auto", bgcolor: "secondary.main", mb: 2, width: 56, height: 56 }}>
          <PersonAddOutlinedIcon />
        </Avatar>
        <Typography component="h1" variant="h4" sx={{ textAlign: "center", fontWeight: 800, mb: 3, letterSpacing: '-0.02em' }}>
          Crear Cuenta
        </Typography>
        <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
          <FormControl fullWidth sx={{ mb: 2 }} variant="outlined" error={Boolean(errors.username)}>
            <InputLabel required>Nombre Usuario</InputLabel>
            <OutlinedInput
              id="username"
              name="username"
              label="Nombre Usuario"
              type="text"
              value={formData.username}
              onChange={handleChange}
            />
            {errors.username ? <FormHelperText>{errors.username}</FormHelperText> : null}
          </FormControl>
          <FormControl fullWidth sx={{ mb: 2 }} variant="outlined" error={Boolean(errors.email)}>
            <InputLabel required>Correo Electrónico</InputLabel>
            <OutlinedInput
              id="email"
              name="email"
              label="Correo Electrónico"
              type="email"
              value={formData.email}
              onChange={handleChange}
            />
            {errors.email ? <FormHelperText>{errors.email}</FormHelperText> : null}
          </FormControl>
          <FormControl fullWidth variant="outlined" error={Boolean(errors.password)}>
            <InputLabel required htmlFor="outlined-adornment-password">
              Contraseña
            </InputLabel>
            <OutlinedInput
              id="outlined-adornment-password"
              name="password"
              type={showPassword ? "text" : "password"}
              value={formData.password}
              endAdornment={
                <InputAdornment position="end">
                  <IconButton
                    aria-label={
                      showPassword
                        ? "hide the password"
                        : "display the password"
                    }
                    onClick={handleClickShowPassword}
                    onMouseDown={(e) => e.preventDefault()} // Simplified
                    onMouseUp={(e) => e.preventDefault()} // Simplified
                    edge="end"
                  >
                    {showPassword ? <VisibilityOff /> : <Visibility />}
                  </IconButton>
                </InputAdornment>
              }
              label="Contraseña"
              onChange={handleChange}
            />
            {errors.password ? <FormHelperText>{errors.password}</FormHelperText> : null}
          </FormControl>

          <Button
            type="submit"
            variant="contained"
            fullWidth
            sx={{ 
              mt: 3, 
              py: 1.5, 
              fontWeight: 700, 
              borderRadius: 3,
              boxShadow: 'none' 
            }}
            disabled={loading}
          >
            {loading ? "Creando cuenta..." : "REGISTRARME"}
          </Button>
        </Box>
        <Grid container justifyContent="center" sx={{ mt: 3 }}>
          <Grid item>
            <Typography component="span" variant="body2" color="text.secondary">¿Ya tienes una cuenta? </Typography>
            <Link component={RouterLink} to="/login" underline="none" sx={{ fontWeight: 700 }}> {/* Removed variant prop */}
              Iniciar Sesión
            </Link>
          </Grid>
        </Grid>
      </Paper>
    </Container>
  );
};

export default RegisterPage;
